...
const OptionField = ({ name, placeholder, value, checked, icon, onChange }: Option) => {
  return (
    <label className={`${inputStyleBase} flex justify-between items-center cursor-pointer`}>
      ...
      <input name={name} value={value} checked={checked} onChange={onChange} type="radio" className="peer sr-only" />
      <span
        className="
